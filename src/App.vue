			...
			<section class="main-content">
				...
				<div v-if="selectedFile" class="file-content-panel">
					...
					<div class="file-tags-section">
						<h3>Теги файла:</h3>
						<div class="tags-list">
							<span v-for="tag in selectedFile.tags" :key="tag" class="tag" @click="handleTagClick(tag)"
								:title="tag">
								#{{ tag }}
							</span>
						</div>
						...
						<!-- Группированные теги -->
						<div v-for="(groupTags, groupName) in selectedFile.groupedTags" :key="groupName"
							class="tag-group">
							<h4>{{ groupName }}:</h4>
							<div class="group-tags">
								<span v-for="tag in groupTags" :key="tag.full" class="group-tag"
									@click="handleTagClick(tag.full)" :title="tag.full">
									{{ tag.display }}
								</span>
							</div>
						</div>
